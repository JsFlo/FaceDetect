...
import com.fhc.emotionrec.facedetect.adapter.FaceEmotionAdapter
...
class FaceDetailEmotionRec(context: Context, attrs: AttributeSet) :
    ...
    fun setPredictionResponse(predictionResponse: PredictionResponse) {
        with(predictionResponse) {
            face_details_emotion_recycler_view.adapter =
                    FaceEmotionAdapter(predictionResponse.sortedPredictions)
